...
// Define RATIOS object globally so all functions can access it
const RATIOS = {
  // 1. Liquidity Ratios
  current_ratio: {
    label: "Current Ratio",
...
// Initialize ratio functionality when DOM is loaded
document.addEventListener('DOMContentLoaded', function() {
  // Only initialize ratio functionality if we're on the ratios page
  if (!document.querySelector('.ratio-calculator-container')) {
    return;
  }
  ...
  window.toggleInputsSummary = function() {
    ...
    const header = inputsSummary.previousElementSibling;
  ...
  if (confirmDeleteYesBtn) {
    confirmDeleteYesBtn.addEventListener('click', function(){
      ...
      const modal = bootstrap.Modal.getInstance(document.getElementById('confirmDeleteSaved'));
      if (modal) modal.hide();
    });
  }

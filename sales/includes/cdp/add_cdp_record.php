...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_cdp'])) {
    ...
    if (empty($title) || empty($cdp_type) || empty($effective_date)) {
        ...
        // Set $employee_id from session (assuming employee_id = user_id for logged-in employee)
        $employee_id = isset($_SESSION['user_id']) ? intval($_SESSION['user_id']) : 0;
        ...
        $insert_query = "INSERT INTO cdp_records 
                ...
                VALUES ($employee_id, '$cdp_type', '$title', '$description', '$document_file', $uplift_percentage, '$effective_date', $user_id, 'PENDING')";

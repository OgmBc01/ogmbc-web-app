...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Check current page and expand appropriate menu
    const currentPath = window.location.pathname;
    const currentFile = currentPath.split('/').pop();
    const urlParams = new URLSearchParams(window.location.search);
    const source = urlParams.get('source');
    
    // Function to expand parent menu based on current page
    function expandCurrentMenu() {
        const menuMappings = {
            'engagements.php': 'engagements-menu',
            'files.php': 'files-menu',
            'support.php': 'support-menu',
            'profile.php': 'profile-menu'
        };
        
        for (const [page, menuId] of Object.entries(menuMappings)) {
            if (currentFile === page) {
                const menu = document.getElementById(menuId);
                const toggle = document.querySelector(`[data-menu="${menuId.replace('-menu', '')}"]`);
                if (menu && toggle) {
    ...
    // Add click event to all menu toggles

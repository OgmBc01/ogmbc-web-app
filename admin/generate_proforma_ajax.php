...
// Include database - use correct path
...
try {
    ...
    // Save proforma to database - FIXED: 10 parameters for 10 values (excluding prepared_at which uses NOW())
    ...
    // FIXED: 10 parameters "iisisdsiss" for 10 values
    // i = client_id (integer)
    // i = proposal_id (integer, can be null)
    // s = invoice_ref (string)
    // i = version (integer)
    // s = invoice_content (string - longtext)
    // d = total_amount (double/decimal)
    // s = payment_breakdown (string - JSON)
    // s = validity_period (string - date)
    // i = prepared_by (integer)
    // s = file_path (string)
    
    $insert_stmt->bind_param(
        "iisisdsiss", // 10 parameters
        $client_id, 
        $proposal_id, 
        $invoice_ref, 
        $version, 
        $html_content,
        $client['service_total_fee'], 
    ...
    if (!$insert_stmt->execute()) {
        throw new Exception('Failed to save proforma to database: ' . $insert_stmt->error);
    }
